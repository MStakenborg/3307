...
int Bank::openAccount(int i){

	string type;
	float initBal;

	cout << "\nAre you opening a chequing or savings account?\n \n Enter:\n 0 for Chequing \n 1 for Savings\n x to Cancel" << "\n\n ";
	cin >> type;
	...
	//Open chequing account
		...
		//chequing account not already open
			...
			if (asn == '0'){
				...
				if (trace){
					traceFile << getTime() << " --- " << userId << " entered invalid input.\n";
				}
				openAccount(i);
			}
		}
		...
		if (clientList[i].getChq().getIni()){
			...
			if (clientList[i].getSav().getIni()){
				...
				// all conditions met, can close account
					...
					clientList[i].setChq(ChqAccount());
					if (trace){
...
void Bank::transferToUser(int i, int user, string type){
...
int Bank::withdrawCheck(int i, string type){
	float checkBal;
...
void Bank::clientOptions(int i){
	userId = clientList[i].getId();
	clientList[i].getNotifications();
	cout << "\nEnter:\n 0 to open a chequing or savings account\n 1 to close an account\n 2 to check balance\n 3 to deposit money\n" 
		<< " 4 to withdraw money \n 5 to transfer funds \n 6 to display transaction history\n "
		<< "7 to display monthly credit card history\n 8 to make credit card payment\n x to logout\n\n ";
	cin >> option;
	...
	// view transaction history
		...
		if (clientList[i].getChq().getIni()){
			// prints transaction history
			clientList[i].getChq().creditHistory();
		}
		else {
...
void Bank::menu(){
	...
	// close program
		...
		if (trace){
			traceFile << getTime() << " --- Unknown user stopped using the bank system.\n";
		}
	}
	// invalid input
	else {
		if (trace){
			traceFile << getTime() << " --- Unknown user entered invalid input in the bank menu.\n\n";
		}
		cout << "\nInvalid input\n\n";
		menu();
	}
				...
				if (clientList[i].payment(0.1 * cheq.getDebt())){
					...
					if (trace){
						traceFile << getTime() << " --- " << clientList[i].getId() << "'s has insufficient funds to pay minimum debt payment."
							<< "Credit card frozen. Manager notified. \n";
						mNotif << "---" << clientList[i].getId() << " had insufficient funds to pay minimum credit debt. Their credit card has been frozen.\n";
					}
				}
			}
		}
